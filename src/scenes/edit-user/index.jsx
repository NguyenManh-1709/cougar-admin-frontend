...
import { userPutAndUploadAvatarToCloud, userPut } from "../../store/apis";
...
const FormEditUser = () => {
  ...
  const { role, ...userSelected } = usersWithRoles.find(item => item.id === idTypeNumber);
  ...
  const handleEditUser = (values) => {
    if (validFile) {
      ...
      dispatch(userPut(values)).then((response) => {
        if (response.type === "userPut/rejected") {
          toast.error(response.payload.message, {
            position: "top-right",
            autoClose: 1000,
            hideProgressBar: false,
  ...
  return (
    <Box m="20px">
      <Header title="EDIT ACCOUNT" subtitle="Account of administrator only" />
